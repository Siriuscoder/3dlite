...
static int initCube(void)
{
    ...
    if (!lite3d_mesh_init(&mCubeVbo))
        return LITE3D_FALSE;
    if (!lite3d_mesh_indexed_load_from_memory(&mCubeVbo, cubeVertices, 24, layout, 2, cubeIndices, 12, 3, LITE3D_VBO_STATIC_DRAW))
        return LITE3D_FALSE;
...
static int init(void *userdata)
{
    ...
    lite3d_scene_node_touch_material(&mSceneNode[0], lite3d_mesh_chunk_get_by_index(&mCubeVbo, 0), &mBoxMaterial, 1);
