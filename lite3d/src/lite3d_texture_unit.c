...
static int8_t max_mipmaps_count(
    int32_t width, int32_t height, int32_t depth, int32_t maxMipmapsSupported)
{
    int8_t count = 0;
...
int lite3d_texture_technique_init(
    const lite3d_texture_technique_settings *settings)
{
    SDL_assert(settings);
    ...
    if (
        !lite3d_check_texture_compression() ||
        !lite3d_check_texture_compression_s3tc() ||
        !lite3d_check_texture_compression_dxt1())
    {
        SDL_LogWarn(
            SDL_LOG_CATEGORY_APPLICATION,
            "%s: Texture compression not supported, skiping",
    ...
    if (!lite3d_check_texture_filter_anisotropic())
    {
        SDL_LogError(
            SDL_LOG_CATEGORY_APPLICATION,
            "%s: EXT_texture_filter_anisotropic not supported...",
            LITE3D_CURRENT_FUNCTION);
...
int lite3d_texture_unit_from_resource(
    lite3d_texture_unit *textureUnit,
    const lite3d_file *resource,
    uint32_t imageType,
    ...
    uint8_t cubeface)
{
    ...
    uint32_t tu_tt = textureUnit->textureTarget;
    ...
    if (
        textureUnit->imageWidth != imageWidth ||
        textureUnit->imageHeight != imageHeight ||
        textureUnit->imageDepth != imageDepth)
    {
        if (!lite3d_texture_unit_allocate(
            textureUnit,
            textureTarget,
            quality,
            wrapping,
    ...
    int8_t bpp = (int8_t) ilGetInteger(IL_IMAGE_BYTES_PER_PIXEL);
    if (textureUnit->imageBPP != bpp)
    {

        SDL_LogWarn(
            SDL_LOG_CATEGORY_APPLICATION,
            "%s: %s: "
            "representable image BPP does not mached with "
            "IL image BPP (%d-%d)",
            (
                tu_tt == LITE3D_TEXTURE_1D ? "TEXTURE_1D" :
                tu_tt == LITE3D_TEXTURE_2D ? "TEXTURE_2D" : "TEXTURE_3D"
            ),
            resource->name,
            textureUnit->imageBPP,
    ...
    if (textureUnit->imageSize != (size_t) ilGetInteger(IL_IMAGE_SIZE_OF_DATA))
    {
        SDL_LogWarn(
            SDL_LOG_CATEGORY_APPLICATION,
            "%s: %s: "
            "representable image size does not mached with "
            "IL image size (%d-%d)",
            (
                tu_tt == LITE3D_TEXTURE_1D ? "TEXTURE_1D" :
                tu_tt == LITE3D_TEXTURE_2D ? "TEXTURE_2D" : "TEXTURE_3D"
    ...
    for (mipLevel = 0; mipLevel <= totalLevels; ++mipLevel)
    {
        ...
        if (!lite3d_texture_unit_set_pixels(
            textureUnit,
            0, 0, 0,
            lWidth,
            lHeight,
    ...
    SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "%s: %s, "
        ...
        "format %s",
        (tu_tt == LITE3D_TEXTURE_1D ? "TEXTURE_1D" :
            tu_tt == LITE3D_TEXTURE_2D ? "TEXTURE_2D" :
              tu_tt == LITE3D_TEXTURE_3D ? "TEXTURE_3D" : "TEXTURE_CUBE"),
        resource->name,
...
int lite3d_texture_unit_set_pixels(
    lite3d_texture_unit *textureUnit,
    int32_t widthOff,
    int32_t heightOff,
    int32_t depthOff,
    ...
    const void *pixels)
{
    ...
    /* make texture active */
    glBindTexture(tu_tt, textureUnit->textureID);
...
int lite3d_texture_unit_set_compressed_pixels(
    lite3d_texture_unit *textureUnit, 
    int32_t widthOff,
    int32_t heightOff,
    int32_t depthOff, 
    ...
    const void *pixels)
{
...
#ifndef GLES
    {
        ...
        if(compressed == GL_FALSE)
        {
            SDL_LogError(
                SDL_LOG_CATEGORY_APPLICATION,
                "%s texture level %d is not a compressed format...",
                LITE3D_CURRENT_FUNCTION,
                level);

            glBindTexture(tu_tt, 0);
...
int lite3d_texture_unit_get_level_size(
    lite3d_texture_unit *textureUnit,
    int8_t level,
    uint8_t cubeface,
    size_t *size)
{
    ...
    imageWidth = lite3d_texture_unit_get_level_width(
        textureUnit, level, cubeface);

    imageHeight = lite3d_texture_unit_get_level_height(
        textureUnit, level, cubeface);

    imageDepth = lite3d_texture_unit_get_level_depth(
        textureUnit, level, cubeface);
...
int lite3d_texture_unit_get_compressed_level_size(
    lite3d_texture_unit *textureUnit, 
    int8_t level,
    uint8_t cubeface,
    size_t *size)
{
    ...
    glGetTexLevelParameteriv(
        (
            tu_tt == LITE3D_TEXTURE_CUBE ?
            GL_TEXTURE_CUBE_MAP_POSITIVE_X + cubeface : tu_tt
        ),
        level,
        GL_TEXTURE_COMPRESSED,
        &compressed);
    ...
    SDL_LogError(
        SDL_LOG_CATEGORY_APPLICATION,
        "%s texture dumping is not supported..",
        LITE3D_CURRENT_FUNCTION);
...
int lite3d_texture_unit_get_pixels(
    lite3d_texture_unit *textureUnit,
    int8_t level,
    uint8_t cubeface,
    void *pixels)
...
int lite3d_texture_unit_get_compressed_pixels(
    lite3d_texture_unit *textureUnit,
    int8_t level,
    uint8_t cubeface,
    void *pixels)
{
    ...
    SDL_LogError(
        SDL_LOG_CATEGORY_APPLICATION,
        "%s texture dumping does not supported...",
        LITE3D_CURRENT_FUNCTION);
    ...
    uint16_t iformat, int32_t width, int32_t height, int32_t depth)
{
    ...
    if (
        ...
        textureTarget != LITE3D_TEXTURE_2D)
    {
        return LITE3D_FALSE;
    }
    ...
    if (quality == LITE3D_TEXTURE_QL_NICEST)
    {
        ...
        glGetTexParameteriv(
            textureTarget, GL_TEXTURE_MAX_LEVEL, &textureMaxLevels);
    ...
    textureUnit->minFilter = (
        quality == LITE3D_TEXTURE_QL_NICEST ? GL_LINEAR_MIPMAP_LINEAR :
        quality == LITE3D_TEXTURE_QL_LOW ? GL_NEAREST : GL_LINEAR);

    textureUnit->magFilter = (
        quality == LITE3D_TEXTURE_QL_LOW ? GL_NEAREST : GL_LINEAR);

    glTexParameteri(
        textureTarget, GL_TEXTURE_MIN_FILTER, textureUnit->minFilter);

    glTexParameteri(
        textureTarget, GL_TEXTURE_MAG_FILTER, textureUnit->magFilter);

    glTexParameteri(
        textureTarget,
        GL_TEXTURE_WRAP_S,
        (
            textureUnit->wrapping == LITE3D_TEXTURE_REPEAT ?
    ...
    switch (textureTarget)
    {
        ...
        case LITE3D_TEXTURE_CUBE:
        {
            ...
            {
                glTexImage2D(
                    GL_TEXTURE_CUBE_MAP_POSITIVE_X + icube,
                    0,
                    internalFormat,
                    width,
                    height,
    ...
    /*
     * You must reserve memory for other mipmaps levels as well either
     * by making a series of calls to glTexImage2D
     * or use glGenerateMipmap(GL_TEXTURE_2D).
     * Here, we'll use :
     */
    if (quality == LITE3D_TEXTURE_QL_NICEST)
    {
    ...
    {
        typedef int (*tsizefunc)(
            lite3d_texture_unit *, int8_t, uint8_t, size_t *);
        ...
        sf = (
            textureUnit->compressed ?
            lite3d_texture_unit_get_compressed_level_size :
    ...
    SDL_LogDebug(SDL_LOG_CATEGORY_APPLICATION, "%s: "
        ...
        (textureUnit->textureTarget == LITE3D_TEXTURE_3D ? "TEXTURE_3D" :
        "TEXTURE_CUBE"))),
        textureUnit->imageWidth,
        textureUnit->imageHeight,
        textureUnit->imageDepth,
...
int32_t lite3d_texture_unit_get_level_width(
    lite3d_texture_unit *textureUnit,
    int8_t level,
    uint8_t cubeface)
{
...
int32_t lite3d_texture_unit_get_level_height(
    lite3d_texture_unit *textureUnit,
    int8_t level,
    uint8_t cubeface)
{
...
int32_t lite3d_texture_unit_get_level_depth(
    lite3d_texture_unit *textureUnit,
    int8_t level,
    uint8_t cubeface)
{
