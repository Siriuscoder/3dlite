...
int ExecuteString(asIScriptEngine *engine, const char *code, void *ref, int refTypeId, asIScriptModule *mod, asIScriptContext *ctx)
{
	...
	lite3dpp::lite3dpp_string funcCode = " ExecuteString() {\n";
	funcCode += code;
	funcCode += "\n;}";
...
int WriteConfigToStream(asIScriptEngine *engine, ostream &strm)
{
	...
	struct Escape
	{
		static lite3dpp::lite3dpp_string Quotes(const char *decl)
		{
			lite3dpp::lite3dpp_string str = decl;
			size_t pos = 0;
			for(;;)
			{
				// Find " characters
				pos = str.find("\"",pos);
				if( pos == lite3dpp::lite3dpp_string::npos )
					break;
	...
	lite3dpp::lite3dpp_string currNamespace = "";
	...
	struct TypeWriter
	{
		static void Write(asIScriptEngine *engine, ostream &strm, asIObjectType *type, lite3dpp::lite3dpp_string &currNamespace, asDWORD &currAccessMask)
		{
			const char *nameSpace = type->GetNamespace();
			if( nameSpace != currNamespace )
			{
			...
			lite3dpp::lite3dpp_string typeDecl = engine->GetTypeDeclaration(type->GetTypeId());
			if( type->GetFlags() & asOBJ_SCRIPT_OBJECT )
			{
...
int ConfigEngineFromStream(asIScriptEngine *engine, istream &strm, const char *configFile)
{
	...
	struct in
	{
		static asETokenClass GetToken(asIScriptEngine *engine, lite3dpp::lite3dpp_string &token, const lite3dpp::lite3dpp_string &text, asUINT &pos)
		{
			asUINT len = 0;
		...
		static void ReplaceSlashQuote(lite3dpp::lite3dpp_string &str)
		{
			size_t pos = 0;
			for(;;)
			{
		...
		static asUINT GetLineNumber(const lite3dpp::lite3dpp_string &text, asUINT pos)
		{
			asUINT count = 1;
	...
	char buffer[1000];
	lite3dpp::lite3dpp_string config;
	do {
	...
	while( pos < config.length() )
	{
		...
		{
			lite3dpp::lite3dpp_string maskStr;
		...
		{
			lite3dpp::lite3dpp_string name, flags;
		...
		{
			lite3dpp::lite3dpp_string name, behaviour, decl;
			in::GetToken(engine, name, config, pos);
			name = name.substr(1, name.length() - 2);
		...
		{
			lite3dpp::lite3dpp_string name, decl;
			in::GetToken(engine, name, config, pos);
			name = name.substr(1, name.length() - 2);
			in::GetToken(engine, decl, config, pos);
			decl = decl.substr(1, decl.length() - 2);
		...
		{
			lite3dpp::lite3dpp_string name, size, flags;
			in::GetToken(engine, name, config, pos);
		...
		{
			lite3dpp::lite3dpp_string type, name, value;
			in::GetToken(engine, type, config, pos);
			in::GetToken(engine, name, config, pos);
		...
		{
			lite3dpp::lite3dpp_string type, decl;
			in::GetToken(engine, type, config, pos);
			in::GetToken(engine, decl, config, pos);
			decl = decl.substr(1, decl.length() - 2);
...
lite3dpp::lite3dpp_string GetExceptionInfo(asIScriptContext *ctx, bool showStack)
{
