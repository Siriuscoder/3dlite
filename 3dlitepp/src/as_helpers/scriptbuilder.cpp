...
// Helper functions
static lite3dpp::lited3dpp_string GetCurrentDir();
static lite3dpp::lited3dpp_string GetAbsolutePath(const lite3dpp::lited3dpp_string &path);
...
lite3dpp::lited3dpp_string CScriptBuilder::GetSectionName(unsigned int idx) const
{
	if( idx >= includedScripts.size() ) return "";

	set<lite3dpp::lited3dpp_string>::const_iterator it = includedScripts.begin();
	while( idx-- > 0 ) it++;
	return *it;
}
...
int CScriptBuilder::AddSectionFromFile(const char *filename)
{
	...
	lite3dpp::lited3dpp_string fullpath = GetAbsolutePath(filename);
...
{
	lite3dpp::lited3dpp_string sword = word;
...
{
	lite3dpp::lited3dpp_string scriptFile = filename;
...
int CScriptBuilder::LoadScriptSection(const char *filename)
{
	...
	if( f == 0 )
	{
		...
		lite3dpp::lited3dpp_string msg = "Failed to open script file '" + GetAbsolutePath(scriptFile) + "'";
	...
	lite3dpp::lited3dpp_string code;
	size_t c = 0;
	if( len > 0 )
	{
		code.resize(len);
	...
	{
		// Write a message to the engine's message callback
		lite3dpp::lited3dpp_string msg = "Failed to load script file '" + GetAbsolutePath(scriptFile) + "'";
		engine->WriteMessage(filename, 0, 0, asMSGTYPE_ERROR, msg.c_str());
		return -1;
	}
...
{
	vector<lite3dpp::lited3dpp_string> includes;
	...
	while( pos < modifiedScript.size() )
	{
		...
		if( t == asTC_UNKNOWN && modifiedScript[pos] == '#' && (pos + 1 < modifiedScript.size()) )
		{
			...
			if( token == "if" )
			{
				...
				{
					lite3dpp::lited3dpp_string word;
...
#if AS_PROCESS_METADATA == 1
	// Preallocate memory
	lite3dpp::lited3dpp_string metadata, declaration;
	...
	while( pos < modifiedScript.size() )
	{
		...
		if( currentNamespace != "" && modifiedScript[pos] == '}' )
		{
			...
			if( found != lite3dpp::lited3dpp_string::npos )
			{
			...
			if( t == asTC_IDENTIFIER )
			{
				...
				if( token == "include" )
				{
					...
					if( t == asTC_VALUE && len > 2 && (modifiedScript[pos] == '"' || modifiedScript[pos] == '\'') )
					{
						...
						lite3dpp::lited3dpp_string includefile;
	...
	if( includes.size() > 0 )
	{
		...
		else
		{
			...
			lite3dpp::lited3dpp_string path = sectionname;
			size_t posOfSlash = path.find_last_of("/\\");
			if( posOfSlash != lite3dpp::lited3dpp_string::npos )
			...
			for( int n = 0; n < (int)includes.size(); n++ )
			{
				...
				if( includes[n].find_first_of("/\\") != 0 &&
					includes[n].find_first_of(":") == lite3dpp::lited3dpp_string::npos )
				{
...
int CScriptBuilder::Build()
{
	...
	for( int n = 0; n < (int)foundDeclarations.size(); n++ )
	{
		...
		if( decl->type == 1 )
		{
			...
			assert( typeId >= 0 );
			if( typeId >= 0 )
				typeMetadataMap.insert(map<int, lite3dpp::lited3dpp_string>::value_type(typeId, decl->metadata));
		}
		else if( decl->type == 2 )
		{
			if( decl->parentClass == "" )
			{
				...
				assert( func );
				if( func )
					funcMetadataMap.insert(map<int, lite3dpp::lited3dpp_string>::value_type(func->GetId(), decl->metadata));
			}
			else
			{
				// Find the method id
				...
				assert( func );
				if( func )
					it->second.funcMetadataMap.insert(map<int, lite3dpp::lited3dpp_string>::value_type(func->GetId(), decl->metadata));
			}
		}
		else if( decl->type == 4 )
		{
		...
		else if( decl->type == 3 )
		{
			if( decl->parentClass == "" )
			{
				...
				assert( varIdx >= 0 );
				if( varIdx >= 0 )
					varMetadataMap.insert(map<int, lite3dpp::lited3dpp_string>::value_type(varIdx, decl->metadata));
			}
			else
			{
				int typeId = module->GetTypeIdByDecl(decl->parentClass.c_str());
				...
				// If found, add it
				assert( idx >= 0 );
				if( idx >= 0 ) it->second.varMetadataMap.insert(map<int, lite3dpp::lited3dpp_string>::value_type(idx, decl->metadata));
			}
		}
	}
...
#if AS_PROCESS_METADATA == 1
int CScriptBuilder::ExtractMetadataString(int pos, lite3dpp::lited3dpp_string &metadata)
{
	metadata = "";
...
int CScriptBuilder::ExtractDeclaration(int pos, lite3dpp::lited3dpp_string &declaration, int &type)
{
	declaration = "";
	type = 0;
	...
	if( t == asTC_KEYWORD || t == asTC_IDENTIFIER )
	{
		...
		else
		{
			...
			pos += len;
			lite3dpp::lited3dpp_string name;
...
{
	map<int,lite3dpp::lited3dpp_string>::iterator it = typeMetadataMap.find(typeId);
...
const char *CScriptBuilder::GetMetadataStringForFunc(asIScriptFunction *func)
{
	if( func )
	{
		map<int,lite3dpp::lited3dpp_string>::iterator it = funcMetadataMap.find(func->GetId());
...
{
	map<int,lite3dpp::lited3dpp_string>::iterator it = varMetadataMap.find(varIdx);
...
const char *CScriptBuilder::GetMetadataStringForTypeProperty(int typeId, int varIdx)
{
	...
	map<int, lite3dpp::lited3dpp_string>::iterator propIt = typeIt->second.varMetadataMap.find(varIdx);
...
const char *CScriptBuilder::GetMetadataStringForTypeMethod(int typeId, asIScriptFunction *method)
{
	if( method )
	{
		...
		map<int, lite3dpp::lited3dpp_string>::iterator methodIt = typeIt->second.funcMetadataMap.find(method->GetId());
...
lite3dpp::lited3dpp_string GetAbsolutePath(const lite3dpp::lited3dpp_string &file)
{
	lite3dpp::lited3dpp_string str = file;

	// If this is a relative path, complement it with the current path
	if( !((str.length() > 0 && (str[0] == '/' || str[0] == '\\')) ||
		  str.find(":") != lite3dpp::lited3dpp_string::npos) )
	{
	...
	size_t pos = 0;
	while( (pos = str.find("\\", pos)) != lite3dpp::lited3dpp_string::npos )
		str[pos] = '/';
	...
	pos = 0;
	while( (pos = str.find("/../")) != lite3dpp::lited3dpp_string::npos )
	{
		size_t pos2 = str.rfind("/", pos-1);
		if( pos2 != lite3dpp::lited3dpp_string::npos )
...
lite3dpp::lited3dpp_string GetCurrentDir()
{
	char buffer[1024];
