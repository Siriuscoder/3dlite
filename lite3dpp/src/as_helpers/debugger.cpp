#include <lite3dpp/as_helpers/debugger.h>
...
lite3dpp::String CDebugger::ToString(void *value, asUINT typeId, bool expandMembers, asIScriptEngine *engine)
{
	if( value == 0 )
		return "<null>";

	lite3dpp::Stringstream s;
	...
	else
	{
		...
		if( value )
		{
			...
			if( it != m_toStringCallbacks.end() )
			{
				...
				lite3dpp::String str = it->second(value, expandMembers, this);
				s << str;
			}
		}
	}
...
bool CDebugger::CheckBreakPoint(asIScriptContext *ctx)
{
	...
	lite3dpp::String file = tmp ? tmp : "";
	size_t r = file.find_last_of("\\/");
	if( r != lite3dpp::String::npos )
...
void CDebugger::TakeCommands(asIScriptContext *ctx)
{
	for(;;)
	{
		...
		if( InterpretCommand(lite3dpp::String(buf), ctx) )
			break;
	}
}

bool CDebugger::InterpretCommand(const lite3dpp::String &cmd, asIScriptContext *ctx)
{
	...
	case 'b':
		{
			// Set break point
			size_t div = cmd.find(':'); 
			if( div != lite3dpp::String::npos && div > 2 )
			{
				lite3dpp::String file = cmd.substr(2, div-2);
				lite3dpp::String line = cmd.substr(div+1);

				int nbr = atoi(line.c_str());

				AddFileBreakPoint(file, nbr);
			}
			else if( div == lite3dpp::String::npos && (div = cmd.find_first_not_of(" \t", 1)) != lite3dpp::String::npos )
			{
				lite3dpp::String func = cmd.substr(div);
	...
	case 'r':
		{
			...
			if( cmd.length() > 2 )
			{
				lite3dpp::String br = cmd.substr(2);
				if( br == "all" )
				{
					m_breakPoints.clear();
...
void CDebugger::PrintValue(const lite3dpp::String &expr, asIScriptContext *ctx)
{
	if( ctx == 0 )
	{
		Output("No script is running\n");
	...
	asUINT len = 0;
	lite3dpp::String scope;
	lite3dpp::String name;
	lite3dpp::String str = expr;
...
void CDebugger::AddFuncBreakPoint(const lite3dpp::String &func)
{
	// Trim the function name
	size_t b = func.find_first_not_of(" \t");
	size_t e = func.find_last_not_of(" \t");
	lite3dpp::String actual = func.substr(b, e != lite3dpp::String::npos ? e-b+1 : lite3dpp::String::npos);
...
void CDebugger::AddFileBreakPoint(const lite3dpp::String &file, int lineNbr)
{
	// Store just file name, not entire path
	size_t r = file.find_last_of("\\/");
	lite3dpp::String actual;
	if( r != lite3dpp::String::npos )
	...
	actual = actual.substr(b, e != lite3dpp::String::npos ? e-b+1 : lite3dpp::String::npos);
...
void CDebugger::Output(const lite3dpp::String &str)
{
