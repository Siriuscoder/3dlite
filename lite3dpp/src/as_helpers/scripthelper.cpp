#include <string.h>
#include <lite3dpp/as_helpers/scripthelper.h>
#include <assert.h>
#include <stdio.h>
#include <fstream>
#include <set>
...
int ExecuteString(asIScriptEngine *engine, const char *code, void *ref, int refTypeId, asIScriptModule *mod, asIScriptContext *ctx)
{
	...
	lite3dpp::String funcCode = " ExecuteString() {\n";
	funcCode += code;
	funcCode += "\n;}";
...
int WriteConfigToStream(asIScriptEngine *engine, ostream &strm)
{
	...
	struct Escape
	{
		static lite3dpp::String Quotes(const char *decl)
		{
			lite3dpp::String str = decl;
			size_t pos = 0;
			for(;;)
			{
				// Find " characters
				pos = str.find("\"",pos);
				if( pos == lite3dpp::String::npos )
					break;
	...
	struct TypeWriter
	{
		static void Write(asIScriptEngine *engine, ostream &strm, asIObjectType *type, lite3dpp::String &currNamespace, asDWORD &currAccessMask)
		{
			const char *nameSpace = type->GetNamespace();
			if( nameSpace != currNamespace )
			{
			...
			lite3dpp::String typeDecl = engine->GetTypeDeclaration(type->GetTypeId());
			if( type->GetFlags() & asOBJ_SCRIPT_OBJECT )
			{
...
int ConfigEngineFromStream(asIScriptEngine *engine, istream &strm, const char *configFile)
{
	...
	struct in
	{
		static asETokenClass GetToken(asIScriptEngine *engine, lite3dpp::String &token, const lite3dpp::String &text, asUINT &pos)
		{
			asUINT len = 0;
		...
		static void ReplaceSlashQuote(lite3dpp::String &str)
		{
			size_t pos = 0;
			for(;;)
			{
		...
		static asUINT GetLineNumber(const lite3dpp::String &text, asUINT pos)
		{
			asUINT count = 1;
	...
	while( pos < config.length() )
	{
		...
		{
			lite3dpp::String name, behaviour, decl;
			in::GetToken(engine, name, config, pos);
			name = name.substr(1, name.length() - 2);
		...
		{
			lite3dpp::String name, size, flags;
			in::GetToken(engine, name, config, pos);
		...
		{
			lite3dpp::String type, name, value;
			in::GetToken(engine, type, config, pos);
			in::GetToken(engine, name, config, pos);
...
lite3dpp::String GetExceptionInfo(asIScriptContext *ctx, bool showStack)
{
	if( ctx->GetState() != asEXECUTION_EXCEPTION ) return "";

	lite3dpp::Stringstream text;
