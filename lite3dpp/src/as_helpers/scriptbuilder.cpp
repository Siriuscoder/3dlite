...
// Helper functions
static lite3dpp::String GetCurrentDir();
static lite3dpp::String GetAbsolutePath(const lite3dpp::String &path);
...
lite3dpp::String CScriptBuilder::GetSectionName(unsigned int idx) const
{
	if( idx >= includedScripts.size() ) return "";

	set<lite3dpp::String>::const_iterator it = includedScripts.begin();
	while( idx-- > 0 ) it++;
	return *it;
}
...
int CScriptBuilder::AddSectionFromFile(const char *filename)
{
	...
	lite3dpp::String fullpath = GetAbsolutePath(filename);
...
{
	lite3dpp::String scriptFile = filename;
...
int CScriptBuilder::LoadScriptSection(const char *filename)
{
	...
	if( f == 0 )
	{
		...
		lite3dpp::String msg = "Failed to open script file '" + GetAbsolutePath(scriptFile) + "'";
	...
	{
		// Write a message to the engine's message callback
		lite3dpp::String msg = "Failed to load script file '" + GetAbsolutePath(scriptFile) + "'";
		engine->WriteMessage(filename, 0, 0, asMSGTYPE_ERROR, msg.c_str());
		return -1;
	}
...
{
	vector<lite3dpp::String> includes;
...
#if AS_PROCESS_METADATA == 1
	// Preallocate memory
	lite3dpp::String metadata, declaration;
	...
	if( includes.size() > 0 )
	{
		...
		else
		{
			...
			lite3dpp::String path = sectionname;
			size_t posOfSlash = path.find_last_of("/\\");
			if( posOfSlash != lite3dpp::String::npos )
			...
			for( int n = 0; n < (int)includes.size(); n++ )
			{
				...
				if( includes[n].find_first_of("/\\") != 0 &&
					includes[n].find_first_of(":") == lite3dpp::String::npos )
				{
...
int CScriptBuilder::Build()
{
	...
	for( int n = 0; n < (int)foundDeclarations.size(); n++ )
	{
		...
		if( decl->type == 1 )
		{
			...
			assert( typeId >= 0 );
			if( typeId >= 0 )
				typeMetadataMap.insert(map<int, lite3dpp::String>::value_type(typeId, decl->metadata));
		}
		else if( decl->type == 2 )
		{
			if( decl->parentClass == "" )
			{
				...
				assert( func );
				if( func )
					funcMetadataMap.insert(map<int, lite3dpp::String>::value_type(func->GetId(), decl->metadata));
			}
			else
			{
				// Find the method id
				...
				assert( func );
				if( func )
					it->second.funcMetadataMap.insert(map<int, lite3dpp::String>::value_type(func->GetId(), decl->metadata));
			}
		}
		else if( decl->type == 4 )
		{
		...
		else if( decl->type == 3 )
		{
			if( decl->parentClass == "" )
			{
				...
				assert( varIdx >= 0 );
				if( varIdx >= 0 )
					varMetadataMap.insert(map<int, lite3dpp::String>::value_type(varIdx, decl->metadata));
			}
			else
			{
				int typeId = module->GetTypeIdByDecl(decl->parentClass.c_str());
				...
				// If found, add it
				assert( idx >= 0 );
				if( idx >= 0 ) it->second.varMetadataMap.insert(map<int, lite3dpp::String>::value_type(idx, decl->metadata));
			}
		}
	}
...
#if AS_PROCESS_METADATA == 1
int CScriptBuilder::ExtractMetadataString(int pos, lite3dpp::String &metadata)
{
	metadata = "";
...
int CScriptBuilder::ExtractDeclaration(int pos, lite3dpp::String &declaration, int &type)
{
	declaration = "";
	type = 0;
...
{
	map<int,lite3dpp::String>::iterator it = typeMetadataMap.find(typeId);
...
const char *CScriptBuilder::GetMetadataStringForFunc(asIScriptFunction *func)
{
	if( func )
	{
		map<int,lite3dpp::String>::iterator it = funcMetadataMap.find(func->GetId());
...
{
	map<int,lite3dpp::String>::iterator it = varMetadataMap.find(varIdx);
...
const char *CScriptBuilder::GetMetadataStringForTypeProperty(int typeId, int varIdx)
{
	...
	map<int, lite3dpp::String>::iterator propIt = typeIt->second.varMetadataMap.find(varIdx);
...
const char *CScriptBuilder::GetMetadataStringForTypeMethod(int typeId, asIScriptFunction *method)
{
	if( method )
	{
		...
		map<int, lite3dpp::String>::iterator methodIt = typeIt->second.funcMetadataMap.find(method->GetId());
...
lite3dpp::String GetAbsolutePath(const lite3dpp::String &file)
{
	lite3dpp::String str = file;

	// If this is a relative path, complement it with the current path
	if( !((str.length() > 0 && (str[0] == '/' || str[0] == '\\')) ||
		  str.find(":") != lite3dpp::String::npos) )
	{
	...
	size_t pos = 0;
	while( (pos = str.find("\\", pos)) != lite3dpp::String::npos )
		str[pos] = '/';
	...
	pos = 0;
	while( (pos = str.find("/../")) != lite3dpp::String::npos )
	{
		size_t pos2 = str.rfind("/", pos-1);
		if( pos2 != lite3dpp::String::npos )
