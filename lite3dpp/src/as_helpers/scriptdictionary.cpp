#include <assert.h>
#include <string.h>
#include <lite3dpp/as_helpers/scriptdictionary.h>
...
CScriptDictionary::CScriptDictionary(asBYTE *buffer)
{
	...
	while( length-- )
	{
		...
		lite3dpp::String name = *(lite3dpp::String*)buffer;
		buffer += sizeof(lite3dpp::String);
...
void CScriptDictionary::EnumReferences(asIScriptEngine *engine)
{
	...
	map<lite3dpp::String, CScriptDictValue>::iterator it;
...
CScriptDictionary &CScriptDictionary::operator =(const CScriptDictionary &other)
{
	...
	map<lite3dpp::String, CScriptDictValue>::const_iterator it;
...
CScriptDictValue *CScriptDictionary::operator[](const lite3dpp::String &key)
{
	// Return the existing value if it exists, else insert an empty value
	map<lite3dpp::String, CScriptDictValue>::iterator it;
	it = dict.find(key);
	if( it == dict.end() )
		it = dict.insert(map<lite3dpp::String, CScriptDictValue>::value_type(key, CScriptDictValue())).first;
	
	return &it->second;
}

const CScriptDictValue *CScriptDictionary::operator[](const lite3dpp::String &key) const
{
...
void CScriptDictionary::Set(const lite3dpp::String &key, void *value, int typeId)
{
	map<lite3dpp::String, CScriptDictValue>::iterator it;
	it = dict.find(key);
	if( it == dict.end() )
...
// different type.
void CScriptDictionary::Set(const lite3dpp::String &key, const asINT64 &value)
{
...
void CScriptDictionary::Set(const lite3dpp::String &key, const double &value)
{
	Set(key, const_cast<double*>(&value), asTYPEID_DOUBLE);
}

// Returns true if the value was successfully retrieved
bool CScriptDictionary::Get(const lite3dpp::String &key, void *value, int typeId) const
{
	map<lite3dpp::String, CScriptDictValue>::const_iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
int CScriptDictionary::GetTypeId(const lite3dpp::String &key) const
{
	map<lite3dpp::String, CScriptDictValue>::const_iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
bool CScriptDictionary::Get(const lite3dpp::String &key, asINT64 &value) const
{
	return Get(key, &value, asTYPEID_INT64);
}

bool CScriptDictionary::Get(const lite3dpp::String &key, double &value) const
{
	return Get(key, &value, asTYPEID_DOUBLE);
}

bool CScriptDictionary::Exists(const lite3dpp::String &key) const
{
	map<lite3dpp::String, CScriptDictValue>::const_iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
void CScriptDictionary::Delete(const lite3dpp::String &key)
{
	map<lite3dpp::String, CScriptDictValue>::iterator it;
	it = dict.find(key);
	if( it != dict.end() )
...
CScriptArray* CScriptDictionary::GetKeys() const
{
	...
	long current = -1;
	std::map<lite3dpp::String, CScriptDictValue>::const_iterator it;
	for( it = dict.begin(); it != dict.end(); it++ )
	{
		current++;
		*(lite3dpp::String*)array->At(current) = it->first;
	}
...
{
	CScriptDictionary *dict = (CScriptDictionary*)gen->GetObject();
	lite3dpp::String *key = *(lite3dpp::String**)gen->GetAddressOfArg(0);
...
void RegisterScriptDictionary_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const ?&in)", asMETHODPR(CScriptDictionary,Set,(const lite3dpp::String&,void*,int),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, ?&out) const", asMETHODPR(CScriptDictionary,Get,(const lite3dpp::String&,void*,int) const,bool), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const int64&in)", asMETHODPR(CScriptDictionary,Set,(const lite3dpp::String&,const asINT64&),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, int64&out) const", asMETHODPR(CScriptDictionary,Get,(const lite3dpp::String&,asINT64&) const,bool), asCALL_THISCALL); assert( r >= 0 );

	r = engine->RegisterObjectMethod("dictionary", "void set(const string &in, const double&in)", asMETHODPR(CScriptDictionary,Set,(const lite3dpp::String&,const double&),void), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, double&out) const", asMETHODPR(CScriptDictionary,Get,(const lite3dpp::String&,double&) const,bool), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("dictionary", "dictionaryValue &opIndex(const string &in)", asMETHODPR(CScriptDictionary, operator[], (const lite3dpp::String &), CScriptDictValue*), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("dictionary", "const dictionaryValue &opIndex(const string &in) const", asMETHODPR(CScriptDictionary, operator[], (const lite3dpp::String &) const, const CScriptDictValue*), asCALL_THISCALL); assert( r >= 0 );
...
const lite3dpp::String &CScriptDictionary::CIterator::GetKey() const 
{ 
	return m_it->first; 
}
