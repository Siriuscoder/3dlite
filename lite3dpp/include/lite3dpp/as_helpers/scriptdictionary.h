...
public:
	...
	void Set(const lite3dpp::String &key, void *value, int typeId);
	void Set(const lite3dpp::String &key, const asINT64 &value);
	void Set(const lite3dpp::String &key, const double &value);

	// Gets the stored value. Returns false if the value isn't compatible with the informed typeId
	bool Get(const lite3dpp::String &key, void *value, int typeId) const;
	bool Get(const lite3dpp::String &key, asINT64 &value) const;
	bool Get(const lite3dpp::String &key, double &value) const;
	...
	CScriptDictValue *operator[](const lite3dpp::String &key);
	const CScriptDictValue *operator[](const lite3dpp::String &key) const;

	// Returns the type id of the stored value, or negative if it doesn't exist
	int GetTypeId(const lite3dpp::String &key) const;

	// Returns true if the key is set
	bool Exists(const lite3dpp::String &key) const;
	...
	// Deletes the key
	void Delete(const lite3dpp::String &key);
...
public:
	...
	public:
		...
		// Accessors
		const lite3dpp::String &GetKey() const;
		int                GetTypeId() const;
	...
	protected:
		...
		CIterator(const CScriptDictionary &dict,
		          lite3dpp::stl<lite3dpp::String, 
                  CScriptDictValue>::map::const_iterator it);

		CIterator &operator=(const CIterator &) {return *this;} // Not used

		lite3dpp::stl<lite3dpp::String, CScriptDictValue>::map::const_iterator m_it;
...
protected:
	...
	lite3dpp::stl<lite3dpp::String, CScriptDictValue>::map dict;
};
